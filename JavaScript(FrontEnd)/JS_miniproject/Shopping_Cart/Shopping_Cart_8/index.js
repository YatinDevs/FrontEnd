...
const createProductElements = productslist.map((e) => {
  /*
       // Product div created
        <div class="product">
        </div>
  ...
  imageElement.classList.add("pimage");
  imageElement.setAttribute("width", "250px");
  imageElement.setAttribute("height", "350px");
  imageElement.setAttribute("src", e.image);
  imageElement.setAttribute("alt", "");

  // Create Para Element
  const para = document.createElement("p");
  para.classList.add("ptitle");
  para.textContent = e.title;

  // Create  priceandButton
  const pricecartDiv = document.createElement("div");
  pricecartDiv.classList.add("priceandaddtocart");

  // Create Price display Para ELement
  const pricePara = document.createElement("p");
  pricePara.classList.add("pprice");
  pricePara.textContent = `${e.price} DH`;
  ...
  productDiv.appendChild(pricecartDiv);

  return productDiv;
});

const displayProducts = () => {
  createProductElements.forEach((productElement) => {
...
class Storage {
  ...
  static addtolocalstorage(product) {
    ...
    const updatedProductList = [...products, product];
...
class Ui {
  static displayproducts(product) {
    const productCartDivContainer = document.querySelector(".pccontainer");

    // Cart Product
    const cartProductDiv = document.createElement("div");
    cartProductDiv.classList.add("cartproduct");

    const pnpDiv = document.createElement("div");
    pnpDiv.classList.add("pnp");

    const imgDiv = document.createElement("div");
    imgDiv.classList.add("img");

    const cartimageElement = document.createElement("img");
    cartimageElement.setAttribute("width", "90px");
    cartimageElement.setAttribute("src", `${product.image}`);
    cartimageElement.setAttribute("alt", "");

    imgDiv.appendChild(cartimageElement);

    const nameandpriceDiv = document.createElement("div");
    nameandpriceDiv.classList.add("nameandprice");

    const titlepara = document.createElement("p");
    titlepara.textContent = product.title;
    const pricepara = document.createElement("p");
    pricepara.textContent = product.price;

    nameandpriceDiv.appendChild(titlepara);
    nameandpriceDiv.appendChild(pricepara);

    pnpDiv.appendChild(imgDiv);
    pnpDiv.appendChild(nameandpriceDiv);
